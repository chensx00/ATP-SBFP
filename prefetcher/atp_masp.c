...
void CACHE::stlb_prefetcher_operate(uint64_t addr, uint64_t ip,
    ...
    if(c0>=128){    //MSB of c0 should be 1 to enable prefetching
        prefetcher_to_use = P1;
        if (prefetcher_to_use == P1) masp_operate(addr, ip, cache_hit, type, prefetch_id, instruction, cpu);
    }
    else{ 
...
void CACHE::print_atp_stats(){
    cout<<"ATP ACCESSED   "<< atp_total<<"   MASP ACTIVE    "<<((double)masp_c/(double)atp_total)*100<<
